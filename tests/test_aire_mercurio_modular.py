#!/usr/bin/env python3
"""
Script de prueba para el extractor modular de Mercurio Aire
"""
import sys
import os
...
def test_extractor():
    try:
        from d4_downloaders.extractorMerc.aire.mercurio_aire_modular import MercurioAireModular
        
        print('🧪 PRUEBA COMPLETA DEL EXTRACTOR MODULAR MERCURIO AIRE')
        print('=' * 60)
        
        # Crear instancia
        extractor = MercurioAireModular(headless=True, company='aire')
            ...
            if 'files' in result and result['files']:
                ...
                for file_path in result['files']:
                    if os.path.exists(file_path):
                        ...
                        # Leer primeras líneas si es un archivo de Excel/CSV
                        try:
                            if file_path.endswith(('.xlsx', '.xls', '.csv')):
                                print(f'  📋 Archivo Excel/CSV detectado')
...
if __name__ == '__main__':
    print("Iniciando prueba del extractor modular de Mercurio Aire...")
