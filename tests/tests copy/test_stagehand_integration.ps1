# Test de Integración Stagehand + Gemini
# =====================================
...
try {
    ...
    Write-TestResult "Node.js disponible" $false "Error verificando Node.js: $($_.Exception.Message)"
    $failedTests++
}
...
if ($healthResult.Success) {
    Write-TestResult "Health Check" $true "Servicio respondiendo correctamente" $healthResult.Data
    $passedTests++
} else {
    Write-TestResult "Health Check" $false "Servicio no disponible" $healthResult.Error
    $failedTests++
    Write-Host "⚠️  El servicio Stagehand Bridge no está corriendo. Iniciándolo..." -ForegroundColor $WarningColor
...
if ($createSessionResult.Success) {
    Write-TestResult "Crear sesión básica" $true "Sesión creada: $sessionId" $createSessionResult.Data
    $passedTests++
    ...
    if ($sessionInfoResult.Success) {
        Write-TestResult "Información de sesión" $true "Sesión activa y funcionando" $sessionInfoResult.Data
        $passedTests++
    } else {
        Write-TestResult "Información de sesión" $false "Error obteniendo info de sesión" $sessionInfoResult.Error
        $failedTests++
    }
    ...
    if ($listSessionsResult.Success) {
        ...
        Write-TestResult "Listar sesiones" $true "$sessionCount sesiones activas" $listSessionsResult.Data
        $passedTests++
    } else {
        Write-TestResult "Listar sesiones" $false "Error listando sesiones" $listSessionsResult.Error
        $failedTests++
    }
    ...
    if ($closeSessionResult.Success) {
        Write-TestResult "Cerrar sesión" $true "Sesión cerrada correctamente" $closeSessionResult.Data
        $passedTests++
    } else {
        Write-TestResult "Cerrar sesión" $false "Error cerrando sesión" $closeSessionResult.Error
        $failedTests++
    }
} else {
    Write-TestResult "Crear sesión básica" $false "Error creando sesión" $createSessionResult.Error
    $failedTests++
}
...
if ($createGeminiSessionResult.Success) {
    Write-TestResult "Crear sesión con Gemini" $true "Sesión con Gemini creada (modo test)" $createGeminiSessionResult.Data
    $passedTests++
    ...
    Write-TestResult "Crear sesión con Gemini" $true "Configuración Gemini manejada correctamente (fallback)" $createGeminiSessionResult.Error
    $passedTests++
}
