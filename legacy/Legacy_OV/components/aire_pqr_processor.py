"""
Procesador específico de PQR para Aire
...
from .aire_pagination_manager import AirePaginationManager
import logging

# Configurar logger específico para este módulo
logger = logging.getLogger('aire_pqr_processor')

# Configurar el logger solo si no tiene handlers
if not logger.handlers:
    handler = logging.StreamHandler()
    formatter = logging.Formatter('[AIRE-PQR] %(levelname)s: %(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)
    logger.setLevel(logging.INFO)
    logger.propagate = False

class AirePQRProcessor:
    """
    Procesador específico de PQR para Aire
    ...
    def __init__(self, page, download_path: str, screenshots_dir: str):
        """
        Inicializa el procesador de PQR de Aire
        ...
        # Si la ruta no termina en el proyecto, construir la ruta correcta
            ...
            base_path = project_root / "data" / "downloads" / "aire" / "oficina_virtual"
        ...
        logger.info(f"Directorio de descarga configurado: {self.download_path.absolute()}")
        logger.info(f"Directorio de datos configurado: {self.data_dir.absolute()}")
        ...
        self.document_proof_selectors = [
            ...
            "td:contains('Documento/prueba') + td a",
            "td:contains('Documento') + td a",
            "td:contains('prueba') + td a",
            ".text-td-label:contains('Documento') ~ td a",
            ".text-td-label:contains('prueba') ~ td a",
            "a[href*='attachment']",
            "a[href*='file']",
            "a[download]",
        ...
        self.pagination_manager = AirePaginationManager(self.download_path.parent)
        
        logger.info("AirePQRProcessor inicializado correctamente")
    ...
    async def process_all_pqr_records(self, max_records: Optional[int] = None, enable_pagination: bool = False) -> int:
        ...
        """
        try:
            logger.info("=== INICIANDO PROCESAMIENTO DE PQR (SECUENCIA ESPECÍFICA AIRE) ===")
            ...
            logger.info(f"PROCESAMIENTO PÁGINA ACTUAL COMPLETADO: {successful_records}/{len(found_buttons)} registros exitosos")
            
            # Si está habilitada la paginación, continuar con páginas siguientes
            if enable_pagination:
                logger.info("Activando paginación automática para procesar todas las páginas...")
                ...
                logger.info(f"PROCESAMIENTO MASIVO COMPLETADO: {total_records_all_pages} registros en total")
    ...
    async def process_current_page_records(self, page, max_records: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error procesando página actual: {e}")
            return {
                'total_processed': 0,
                'successful': 0,
    ...
    async def _process_single_pqr_specific_sequence(self, eye_button, record_number: int) -> bool:
        ...
        """
        try:
            logger.info(f"INICIANDO SECUENCIA ESPECÍFICA PARA PQR #{record_number}")
            ...
            if not new_page:
                logger.error("No se pudo abrir nueva pestaña")
                return False
            ...
            try:
                ...
                try:
                    ...
                    logger.info("Nueva pestaña cargada exitosamente")
                except Exception as load_error:
                    logger.warning(f"Timeout en networkidle, intentando con domcontentloaded: {load_error}")
                    try:
                        await new_page.wait_for_load_state('domcontentloaded', timeout=30000)  # 30 segundos
                        logger.info("Nueva pestaña cargada con domcontentloaded")
                        # Esperar adicional para asegurar carga de AngularJS
                        await new_page.wait_for_timeout(5000)
                    except Exception as fallback_error:
                        logger.warning(f"Timeout en domcontentloaded también, continuando: {fallback_error}")
                        # Esperar un poco y continuar
                        await new_page.wait_for_timeout(3000)

                # PASO 2: Extraer número SGC para nombrar archivos
                logger.info("PASO 2: Extrayendo número SGC...")
                sgc_number = await self._extract_sgc_number_from_page(new_page, record_number)
                logger.info(f"Número SGC extraído: {sgc_number}")

                # PASO 3: Generar PDF con nombre del SGC
                logger.info("PASO 3: Generando PDF con nombre del SGC...")
                pdf_success = False
                try:
                    pdf_success = await self._generate_pdf_with_sgc_name(new_page, sgc_number, record_number)
                except Exception as pdf_error:
                    logger.error(f"Error en generación PDF: {pdf_error}")
                ...
                try:
                    ...
                    logger.error(f"Error en descarga adjuntos: {attachments_error}")

                # PASO 5: Extraer y guardar JSON con todos los datos
                logger.info("PASO 5: Extrayendo y guardando JSON...")
                json_success = False
                try:
                    json_success = await self._extract_and_save_json_data(new_page, sgc_number, record_number)
                except Exception as json_error:
                    logger.error(f"Error en extracción JSON: {json_error}")
                ...
                logger.info(f"Resultados PQR #{record_number}: PDF={pdf_success}, Adjuntos={attachments_success}, JSON={json_success}")
                
                if overall_success:
                    logger.info(f"Secuencia completada para PQR #{record_number}")
                else:
                    logger.warning(f"Ningun paso fue exitoso para PQR #{record_number}")
                ...
                try:
                    if new_page and not new_page.is_closed():
                        ...
                        logger.info("Pestaña cerrada, continuando con siguiente registro")
                    else:
                        logger.info("Pestaña ya estaba cerrada")
                except Exception as close_error:
                    logger.warning(f"Error cerrando pestaña: {close_error}")

        except Exception as e:
            logger.error(f"Error en secuencia específica para PQR {record_number}: {e}")
            return False
    ...
    async def _open_new_tab_without_closing_current(self, eye_button) -> Optional[Any]:
        ...
        try:
                ...
                for option in context_menu_options:
                    try:
                        ...
                        if await menu_item.is_visible(timeout=2000):
                            ...
                            logger.info(f"Menú contextual usado: {option}")
                            ...
                            if current_pages > initial_pages:
                                ...
                                logger.info(f"Nueva pestaña abierta ({current_pages} pestañas)")
                                return new_page
                            break
                    except Exception:
                        continue
            ...
            # MÉTODO 2: Ctrl+Click
            try:
                logger.info("Método 2: Ctrl+Click...")
                ...
                if current_pages > initial_pages:
                    new_page = context.pages[-1]
                    logger.info(f"Nueva pestaña abierta con Ctrl+Click ({current_pages} pestañas)")
                    return new_page

            except Exception as ctrl_error:
                logger.warning(f"Error con Ctrl+Click: {ctrl_error}")

            logger.error("No se pudo abrir nueva pestaña con ningún método")
            return None
    ...
    async def _extract_sgc_number_from_page(self, page, record_number: int) -> str:
        ...
        try:
            logger.info("Extrayendo número SGC...")
                ...
                try:
                    ...
                    if await element.count() > 0:
                        ...
                        if value and value.strip():
                            ...
                            logger.info(f"SGC encontrado con selector {selector}: {sgc_number}")
                            return sgc_number
            ...
            logger.warning(f"No se encontró número SGC, usando valor por defecto: {default_sgc}")
            return default_sgc
    ...
    async def _generate_pdf_with_sgc_name(self, page, sgc_number: str, record_number: int) -> bool:
        ...
        try:
            logger.info(f"Generando PDF con nombre SGC: {sgc_number}")
    ...
    async def _download_document_proof_attachments(self, page, sgc_number: str, record_number: int) -> bool:
        ...
        Método mejorado basado en Afinia
        ...
        """
        try:
            logger.info("[EMOJI_REMOVIDO] Verificando adjuntos en 'Documento/prueba'...")
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            logger.info(f"[EMOJI_REMOVIDO] Archivo documento_prueba extraído: {documento_prueba_filename}")
            ...
            try:
                ...
                logger.info(f"[EMOJI_REMOVIDO] Screenshot para análisis de adjuntos: {screenshot_path}")
            except Exception:
                pass

            # Si tenemos el nombre del archivo, intentar descargarlo
            if documento_prueba_filename and documento_prueba_filename != "":
                logger.info(f"[RESULTADO] Buscando enlace de descarga: {documento_prueba_filename}")
                    ...
                    try:
                        ...
                        for i in range(count):
                            ...
                            if is_visible and documento_prueba_filename in element_text:
                                logger.info(f"   [RESULTADO] ¡ENCONTRADO! Elemento con archivo: {documento_prueba_filename}")
                                try:
                                    logger.info(f"   [EMOJI_REMOVIDO] Intentando descarga con selector: {selector}")
                                    
                                    # Esperar un poco antes del clic
                                    ...
                                    logger.info(f"   [DATOS] Tamaño: {attachment_path.stat().st_size} bytes")
                                    return True
                ...
                logger.info("[EMOJI_REMOVIDO] Intentando método específico para AngularJS...")
                try:
                    angular_selectors = [
                    ...
                    for angular_selector in angular_selectors:
                        try:
                            logger.info(f"[EMOJI_REMOVIDO] Probando selector AngularJS: {angular_selector}")
                            angular_elements = page.locator(angular_selector)
                            angular_count = await angular_elements.count()
                            
                            logger.info(f"[EMOJI_REMOVIDO] Encontrados {angular_count} elementos AngularJS")
                            ...
                            for i in range(angular_count):
                                ...
                                if angular_visible and documento_prueba_filename in angular_text:
                                    logger.info(f"   [RESULTADO] ¡ENCONTRADO! Elemento AngularJS con archivo")
                                    
                                    try:
                                        # Intentar con expectativa de descarga
                                        logger.info(f"   [EMOJI_REMOVIDO] Clic en elemento AngularJS con expectativa...")
                                        async with page.expect_download(timeout=30000) as download_info:
                                        ...
                                        try:
                                            logger.info(f"   [EMOJI_REMOVIDO] Clic simple en elemento AngularJS...")
                                            await angular_element.click()
                                            await page.wait_for_timeout(5000)  # Esperar respuesta
                                            logger.info(f"   [INFO] Clic AngularJS ejecutado (sin descarga detectada)")
                ...
                logger.info("[INFO] No se pudo descargar con selectores directos ni AngularJS")
                if documento_prueba_filename:
                    logger.info(f"[INFO] Archivo buscado: {documento_prueba_filename}")
                return False
                        
            logger.info("No se encontraron adjuntos descargables")
            return False
    ...
    async def _extract_documento_prueba_filename(self, page) -> str:
        ...
        try:
            ...
            documento_selectors = [
                ...
                "td:contains('Documento/prueba') + td",
                "td:contains('Documento') + td",
                ".text-td-label:contains('Documento') + td"
            ]
                    ...
                    if count > 0:
                        for i in range(count):
                            ...
                            if match:
                                ...
                                logger.info(f"[ARCHIVO] Archivo encontrado en campo: {filename}")
                                return filename
                            ...
                            if '.' in text and len(text.strip()) < 100:  # Probable nombre de archivo
                                ...
                                if not any(word in clean_text.lower() for word in ['campo', 'documento', 'prueba', 'adjunto']):
                                    logger.info(f"[ARCHIVO] Posible archivo encontrado: {clean_text}")
                                    return clean_text
    ...
    async def _extract_and_save_json_data(self, page, sgc_number: str, record_number: int) -> bool:
        ...
        try:
            logger.info(f"Extrayendo datos JSON para PQR #{record_number}")
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            logger.info(f"JSON guardado exitosamente: {json_path}")
            logger.info(f"Campos extraídos: {len([k for k, v in pqr_data.items() if v])}")
            
            return True

        except Exception as e:
            logger.error(f"Error extrayendo/guardando JSON: {e}")
            return False