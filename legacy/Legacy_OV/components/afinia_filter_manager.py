...
"""
Gestor de filtros específico para Afinia
...
logger = logging.getLogger('AFINIA-FILTER')

class AfiniaFilterManager:
    """
    Gestor de filtros específico para Afinia
    ...
    async def configure_filters_and_search(self, days_back: int = 1) -> bool:
        ...
        try:
            ...
            self.logger.info("EXITOSO Configuración de filtros y búsqueda completada exitosamente")
            return True
            
        except Exception as e:
            self.logger.error(f"ERROR Error en configuración de filtros: {str(e)}")
            return False
    ...
    async def _expand_filters_panel(self) -> bool:
        ...
        try:
            ...
            for i, selector in enumerate(filter_button_selectors):
                try:
                    ...
                    if element and await element.is_visible():
                        ...
                        self.logger.info(f"EXITOSO Botón de filtrar encontrado con selector: {selector}")
                        break
            ...
            self.logger.info("ESPERANDO Esperando expansión del panel...")
            await asyncio.sleep(2)
            
            self.logger.info("EXITOSO Panel de filtros expandido exitosamente")
            return True
    ...
    async def _configure_status_filter(self) -> bool:
        ...
        try:
            ...
            for i, selector in enumerate(status_selectors):
                try:
                    ...
                    if element and await element.is_visible():
                        ...
                        self.logger.info(f"EXITOSO Campo de estado encontrado con selector: {selector}")
                        break
                except PlaywrightTimeoutError:
                    self.logger.info(f"Selector {selector} no encontrado")
                    continue
            ...
            for i, selector in enumerate(finalizado_selectors):
                try:
                    ...
                    if element and await element.is_visible():
                        ...
                        self.logger.info(f"EXITOSO Opción 'Finalizado' encontrada con selector: {selector}")
                        break
                except PlaywrightTimeoutError:
                    self.logger.info(f"Selector {selector} no encontrado")
                    continue
            ...
            self.logger.info("EXITOSO Estado 'Finalizado' configurado exitosamente")
            return True
    ...
    async def _configure_date_filters(self, days_back: int = 1) -> bool:
        ...
        try:
            ...
            if initial_success or final_success:
                ...
                self.logger.warning("ADVERTENCIA No se pudieron configurar las fechas completamente")
                return False
        ...
        try:
            ...
            for selector in filter_toggle_selectors:
                try:
                    ...
                    if element and await element.is_visible():
                        await element.click()
                        self.logger.info("EXITOSO Panel de filtros colapsado")
                        await asyncio.sleep(1)
                        return True
                except:
                    continue
                    
            self.logger.info("ℹ No se pudo colapsar el panel (no crítico)")
            return True
