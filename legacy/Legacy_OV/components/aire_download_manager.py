...
"""
Gestor de descargas específico para Aire
...
class AireDownloadManager:
    ...
    async def execute_search_and_process(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Búsqueda y procesamiento completados exitosamente")
            return True
            
        except Exception as e:
            self.logger.error(f"Error en búsqueda y procesamiento: {str(e)}")
            return False
    ...
    async def _process_search_results(self) -> bool:
        ...
        try:
            ...
            if extracted_data:
                self.logger.info(f"Extraídos {len(extracted_data)} registros")
                ...
                try:
                    ...
                    if table:
                        ...
                        if len(rows) > 1:  # Más de 1 fila (header + datos)
                            self.logger.info(f"Tabla encontrada con {len(rows)} filas")
                            return True
                except:
                    continue
    ...
    async def _save_extracted_data(self, data: List[dict]) -> bool:
        ...
        try:
            ...
            filename = f"aire_pqr_data_{timestamp}.json"
